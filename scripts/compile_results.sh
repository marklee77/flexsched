...
for RESULT_FILE in ${INDIR}/prob-*.result; do
    ...
    CLASSES=$(${SCRIPTDIR}/count_machine_classes.py ${INDIR}/${RESULT_PREFIX}.prob)
    RCOV_FILE=${OUTDIR}/realcovs-${NUMHOSTS}-${NUMJOBS}-${SLACK}.txt
    echo -e "${KEY}\t${COV}\t${INDEX}\t${REALCOV}\t${CLASSES}" >> ${RCOV_FILE}
...
for RCOV_FILE in ${OUTDIR}/realcovs-*.txt; do
    ...
    echo -e "key\tcov\tindex\trcov\tclasses" > ${RCOV_SORT_FILE}
    sort ${RCOV_FILE} >> ${RCOV_SORT_FILE}
    rm ${RCOV_FILE}
    ITER=0
    FIELDS="1.1 1.2 1.3 1.4"
    ...
    mv ${JOIN_FILE} ${OUTDIR}/minyield-vs-cov-${NUMHOSTS}-${NUMJOBS}-${SLACK}.txt
done
