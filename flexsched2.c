...
int MILP_scheduler(char *ignore1, char *ignore2, char *ignore3)
{
  ...
  // Solve it
  if (solve_linear_program(INTEGER, NOT_VERBOSE, prob)) {
    return RESOURCE_ALLOCATION_FAILURE;
  } 
...
int GREEDY_pick_server_P1(int service)
{
  ...
  for (i=0; i<INS.numservers; i++) {
    if (!service_can_fit_on_server(service,i)) {
      continue;
    }
    load = compute_server_load_in_dimension(
...
int GREEDY_pick_server_P2(int service)
{
  ...
  for (i=0; i<INS.numservers; i++) {
    ...
    load = compute_sum_server_load(i, "fluid"); 
    if ((minload == -1.0) || (load < minload)) {
      minload = load;
      picked = i;
    }
...
int GREEDY_pick_server_P3_P5(int service, const char *mode)
{
  ...
  for (i=0; i<INS.numservers; i++) {
    ...
    if (array_max(INS.rigidneeds[service],INS.numrigid) > 
      ...
      load = compute_server_load_in_dimension(i, "rigid",
                           array_argmax(INS.rigidneeds[service],INS.numrigid));
    } else {
      load = compute_server_load_in_dimension(i, "fluidmin",
...
int GREEDY_pick_server_P4_P6(int service, const char *mode)
{
  ...
  for (i=0; i<INS.numservers; i++) {
    ...
    load = compute_sum_server_load(i, "rigid") +
           compute_sum_server_load(i, "fluidmin");
  ...
  for (i=0; i < vp->num_vectors; i++) {
    if (!strcmp(fit_type, "FIRST")) { // First Fit
      for (j=0; j<vp->num_vectors; j++) { // At worst, one bin per job
        if (vp_vector_can_fit_in_bin(vp, i, j)) {
          vp->mapping[i] = j;        
	  if (j > max_bin)
            max_bin = j;
          break;
...
glp_prob *CHERUKI_create_lp(struct vp_instance *vp, int m)
{
  glp_prob *prob;
  int i,j,k;
  ...
  glp_term_hook(&glp_stderr_out, NULL);
  prob = glp_create_prob();
  glp_set_prob_name(prob, "CHERUKI task placement");
...
/* CHERUKI_process_integer_assignments() */
int CHERUKI_process_integer_assignments(
...
#if 0
  fprintf(stderr,"CHERUKI: num integers = %d\n",num_ints);
  fprintf(stderr,"CHERUKI: num non-integers = %d\n",vp->num_vectors * vp->num_bins - num_ints);
  fprintf(stderr,"CHERUKI: Max num of non-integers = %d\n",vp->num_bins * vp->num_dims);
#endif
  // Sanity check 
  if (vp->num_vectors * vp->num_bins - num_ints > vp->num_dims * vp->num_bins) {
    fprintf(stderr,"CHERUKI: There are two many non-integer variables values!\n");
    return 1;
  }
...
/* CHERUKI_get_unmapped_vectors() */
int CHERUKI_get_unmapped_vectors(struct vp_instance *vp, 
...
//  fprintf(stderr,"*############# %d %d\n",vp->num_dims, num_remaining_unmapped_vectors);
  ...
  fprintf(stderr,"CHERUKI: find_maximum_subset(): Can't find a subset\n");
  return 0;
}

/* CHERUKI_process_non_integer_assignments() */
int CHERUKI_process_non_integer_assignments(struct vp_instance *vp)
{
  int *remaining_unmapped_vectors;
  int num_remaining_unmapped_vectors;
  int k;
  ...
  while (num_remaining_unmapped_vectors = 
             CHERUKI_get_unmapped_vectors(vp, &remaining_unmapped_vectors)) {
    int subset_size;
    int *subset;
...
int solve_vp_instance_CHERUKI(struct vp_instance *vp)
{
  glp_prob *prob = NULL;
  ...
  while(m_hi != m_lo) {
   ...
   // fprintf(stderr,"Trying with %d bins (lo=%d  hi=%d)\n",m,m_lo,m_hi);
    ...
    prob = CHERUKI_create_lp(vp,m);

    //fprintf(stderr,"Writing CHERUKI LP to file 'linearprogram'..");
    glp_write_lp(prob,NULL,"linearprogram");
    //fprintf(stderr,"done\n");
  ...
  // integer assignments
  if (CHERUKI_process_integer_assignments(vp,last_solved_prob)) {
    fprintf(stderr,"CHERUKI: cannot process integer assignments\n");
  ...
  if (CHERUKI_process_non_integer_assignments(vp)) {
    fprintf(stderr,"CHERUKI: cannot process integer assignments\n");
    return VP_NOT_SOLVED;
  }
                      ...
                      char *vp_algorithm)
{
  ...
  } else if (!strcmp(vp_algorithm,"CHERUKI")) {
    status = solve_vp_instance_CHERUKI(vp);
  } else {
...
void AZAR1_scheduler(
    int hosts, int tasks, float *taskcpu, float *taskmem, // Input
    int *taskhost                                         // Output
) {
    int i, j, t, h;
    float cpuload[hosts], memload[hosts];
    int taskmap[tasks];
    float taskkey[tasks], hostkey[hosts];
    int s1map[hosts], s2map[hosts], s3map[hosts], s4map[hosts];
    int s1count, s2count, s3count, s4count;
    float alpha = (2.0*hosts - 2) / (3.0*hosts + 1);

    for (i = 0; i < tasks; i++) {
        taskmap[i] = i;
        if (taskcpu[i] > taskmem[i]) {
            taskkey[i] = taskcpu[i];
        } else {
            taskkey[i] = taskmem[i];
        }
    }
    // online alg, will try semi-on-line later
    sortmap(taskmap, taskkey, tasks, SORT_DESC);
    ...
    for (i = 0; i < tasks; i++) {
        ...
        for (j = 0; j < hosts; j++) {
            if (taskmem[t] + memload[j] <= 1.0) { // only consider hosts that
                                                  // won't violate memory const.
                if (cpuload[j] + taskcpu[t] <= alpha) {
                    // s1 is short hosts that remain short
                    s1map[s1count] = j;
                    s1count++;
                } else if (cpuload[j] + taskcpu[t] <= 1 + alpha) {
                    if (cpuload[j] <= alpha) {
                        // s2 is short hosts that become tall but not
                        // too tall
                        s2map[s2count] = j;
                        s2count++;
                    } else {
                        // s3 is tall hosts that do not become too tall
                        s3map[s3count] = j;
                        s3count++;
                    }
                } else {
                    // paper doesn't describe s4, but might be neceesry to
                    // use if all other sets are empty
                    // s4 is too tall hosts and tall hosts that become too tall
                    s4map[s4count] = j;
                    s4count++;
                }
            }
        ...
        if (s3count) {
            taskhost[t] = s3map[0];
            cpuload[s3map[0]] += taskcpu[t];
            memload[s3map[0]] += taskmem[t];
        } else if (s1count) {
            // want to make sure not to pick an empty host first
            sortmap(s1map, cpuload, s1count, SORT_DESC);
            taskhost[t] = s1map[0];
            cpuload[s1map[0]] += taskcpu[t];
            memload[s1map[0]] += taskmem[t];
        } else if (s2count) {
            // put on least loaded machine
            sortmap(s2map, cpuload, s2count, SORT_ASC);
            taskhost[t] = s2map[0];
            cpuload[s2map[0]] += taskcpu[t];
            memload[s2map[0]] += taskmem[t];
        // this part isn't in the orig alg, will uncomment if failure rate
        // is abysmal
        } else if (s4count) {
            // going outside the alg, but might as well *try*
            sortmap(s4map, cpuload, s4count, SORT_ASC);
            taskhost[t] = s4map[0];
            cpuload[s4map[0]] += taskcpu[t];
            memload[s4map[0]] += taskmem[t];
        } else { // nada
            break;
        }
    }
...
int GB_helper(
    int hosts, int tasks,
    float *taskcpu, float *taskmem, int *taskhost,
    int t, float *cpuload, float *memload
)
{
    int i, j, h;
    int hostmap[hosts];

    if (callcount++ > 500000) return 0;

    for (i = 0; i < hosts; i++) {
        hostmap[i] = i;
    }
    sortmap(hostmap, cpuload, hosts, SORT_ASC);

    for (i = 0; i < hosts; i++) {
        h = hostmap[i];
        if(memload[h] + taskmem[t] <= 1.0) {
            taskhost[t] = h;
            cpuload[h] += taskcpu[t];
            memload[h] += taskmem[t];
            if(
                t+1 == tasks ||
                GB_helper(hosts, tasks, taskcpu, taskmem, taskhost, t+1,
                    cpuload, memload)
            ) {
                return 1;
            }
...
void GB_scheduler(
    ...
    if (!GB_helper(hosts, tasks, taskcpu, taskmem, taskhost, 0, cpuload,
            memload)) {
        for (i = 0; i < tasks; i++) {
            taskhost[i]  = -1;
        }
...
int SGB_helper(
    int hosts, int tasks, int *taskmap,
    float *taskcpu, float *taskmem, int *taskhost,
    int n, float *cpuload, float *memload
)
{
    int t = taskmap[n];
    int i, j, h;
    ...
    for (i = 0; i < hosts; i++) {
        ...
        if(memload[h] + taskmem[t] <= 1.0) {
            ...
            if(
                n+1 == tasks ||
                SGB_helper(hosts, tasks, taskmap, taskcpu, taskmem, taskhost,
                    n+1, cpuload, memload)
            ) {
                return 1;
            }
...
void SGB_scheduler(
    ...
    sortmap(taskmap, taskmem, tasks, SORT_DESC);

    for (i = 0; i < hosts; i++) {
        cpuload[i] = 0.0;
        memload[i] = 0.0;
    }
    if (!SGB_helper(hosts, tasks, taskmap, taskcpu, taskmem, taskhost, 0,
            cpuload, memload)
    ) {
        for (i = 0; i < tasks; i++) {
            taskhost[i]  = -1;
...
float calc_optyield(int hosts, int tasks, float *taskcpu) {
    float totalcpu = 0.0;
    int i;
    for (i = 0; i < tasks; i++) {
        totalcpu += taskcpu[i];
    ...
    if (totalcpu <= (float)hosts) {
        return 1.0;
    } 

    return (float)hosts / totalcpu;
}
...
void SOS_scheduler(
    ...
    // FIXME: really should discretize everything to begin with...
    float yieldub = calc_optyield(hosts, tasks, taskcpu);
    float yieldlb = 0.0;
    float yield, bestyield = 0.0;
    int staskhost[tasks];
    int i, j, t, h;
    float cpuload[hosts], memload[hosts];
    int taskmap[tasks], hostmap[hosts];
    float taskkey[tasks], hostkey[hosts];
    int phosts;
    // FIXME: make dynamic, sqrt of num of hosts
    int levels = 65;
    int N[levels][levels];
    int ocl, oml, ncl, nml;
    ...
    while(yieldub - yieldlb > 0.001) {
        ...
        for (i = 0; i < tasks; i++) {
            taskmap[i] = i;
            if (yield * taskcpu[i] > taskmem[i]) {
                taskkey[i] = yield * taskcpu[i];
            } else {
                taskkey[i] = taskmem[i];
            }
        }
        ...
        for (i = 0; i < tasks; i++) {
            ...
            for (j = 0; j < hosts; j++) {
                if (
                    cpuload[j] + yield * taskcpu[t] <= 1.0 &&
                    memload[j] + taskmem[t] <= 1.0
                ) {
                    hostmap[phosts] = j;
                    phosts++;
                    ocl = levels * (0.999 * cpuload[j]);
                    oml = levels * (0.999 * memload[j]);
                    ncl = levels * (0.999 * (cpuload[j] + yield * taskcpu[t]));
                    nml = levels * (0.999 * (memload[j] + taskmem[t]));
                    hostkey[j] = 0.0;
                    if (ocl != ncl || oml != nml) {
                        if (ocl > 0.0 || oml > 0.0) {
                            hostkey[j] += 1.0 - 2.0 * N[ocl][oml];
                        }
                        if (ncl < (levels - 1) && nml < (levels -1)) {
                            hostkey[j] += 1.0 + 2.0 * N[ncl][nml];
                        }
                    }
                }
            }
            ...
            // FIXME: really just need minimum...
            sortmap(hostmap, hostkey, phosts, SORT_ASC);
            h = hostmap[0];
            taskhost[t] = h;
            ocl = levels * (0.999 * cpuload[j]);
            oml = levels * (0.999 * memload[j]);
            N[ocl][oml]--;
            cpuload[h] += yield * taskcpu[t];
            memload[h] += taskmem[t];
            ncl = levels * (0.999 * cpuload[j]);
            nml = levels * (0.999 * memload[j]);
            N[ncl][nml]++;
        }
...
// Multi-Capacity Binpacking based on Leinberger, breaks tasks into c and m
// types and tries to balance capacities
void MCB_scheduler(
    int hosts, int tasks, float *taskcpu, float *taskmem, int skeytype, //Input
    int *taskhost                                                       //Output
) {
    int i, j, k, t;
    float yieldub = calc_optyield(hosts, tasks, taskcpu);
    ...
    while(yieldub - yieldlb > 0.001) {
        ...
        for (i = 0; i < tasks; i++) {
            ...
            // partition tasks into c and m types
            if (yield * taskcpu[i] > taskmem[i]) {
                taskmap[ctasks] = i;
                ctasks++;
            } else {
                mtasks++;
                taskmap[tasks - mtasks] = i;
            }
            ...
            switch (skeytype) {
                case 1:
                    ...
                    sortkeys[i] = (yield * taskcpu[i]) + taskmem[i];
                    break;
                case 2:
                    // difference of dominant and subordinate
                    if (yield * taskcpu[i] < taskmem[i]) {
                        sortkeys[i] = taskmem[i] - yield * taskcpu[i];
                    } else {
                        sortkeys[i] = yield * taskcpu[i] - taskmem[i];
                    }
                    break;
                case 3:
                    // ratio of dominant to subordinate
                    if (yield * taskcpu[i] > taskmem[i]) {
                        sortkeys[i] = yield * taskcpu[i] / taskmem[i];
                    } else {
                        sortkeys[i] = taskmem[i] / (yield * taskcpu[i]);
                    ...
                    break;
                case 4:
                    // dominant attribute only
                    if (yield * taskcpu[i] < taskmem[i]) {
                        sortkeys[i] = taskmem[i];
                    } else {
                        sortkeys[i] = yield * taskcpu[i];
                    }
                    break;
                case 5:
                    // inverse of sum
                    sortkeys[i] = 1.0 / ((yield * taskcpu[i]) + taskmem[i]);
                    break;
                case 6:
                    // difference of subordinate and dominant
                    if (yield * taskcpu[i] < taskmem[i]) {
                        sortkeys[i] = yield * taskcpu[i] - taskmem[i];
                    } else {
                        sortkeys[i] = taskmem[i] - yield * taskcpu[i];
                    ...
                    break;
                case 7:
                    // ratio of subordinate to dominant
                    if (yield * taskcpu[i] > taskmem[i]) {
                        sortkeys[i] = taskmem[i] / (yield * taskcpu[i]);
                    } else {
                        sortkeys[i] = yield * taskcpu[i] / taskmem[i];
                    ...
                    break;
                case 8:
                    // inverse of dominant attribute
                    if (yield * taskcpu[i] < taskmem[i]) {
                        sortkeys[i] = 1.0 / taskmem[i];
                    } else {
                        sortkeys[i] = 1.0 / (yield * taskcpu[i]);
                    }
                    break;
                case 9:
                default:
                    // inverse of euclidean norm
                    sortkeys[i] = 1.0 / (
                            (yield * taskcpu[i]) * (yield * taskcpu[i]) +
                            taskmem[i] * taskmem[i]
                    );
                    break;
            } 

            // initially tasks are not assigned to hosts
            taskhost[i] = -1;
        }

        // reset host cpu and memory load to 0.0
        for (i = 0; i < hosts; i++) {
            cpuload[i] = memload[i] = 0.0;
        }

        for (i = 0; i < hosts && (ctasks || mtasks); i++) {
            j = k = 0;
            while (j < ctasks || k < mtasks) {
                if ((cpuload[i] < memload[i] && j < ctasks) || k >= mtasks) {
                    sortmap(taskmap + j, sortkeys, ctasks - j, SORT_ASC);
                    while (j < ctasks) {
                        t = taskmap[j];
                        if (
                            cpuload[i] + yield * taskcpu[t] <= 1.0 &&
                            memload[i] + taskmem[t] <= 1.0
                        ) {
                            ctasks--;
                            taskmap[j] = taskmap[ctasks];
                            cpuload[i] += yield * taskcpu[t];
                            memload[i] += taskmem[t];
                            taskhost[t] = i;
                            break;
                        }
                    ...
                    sortmap(taskmap + tasks - mtasks, sortkeys, mtasks - k,
                        SORT_DESC);
                    while (k < mtasks) {
                        t = taskmap[tasks - 1 - k];
                        if (
                            ...
                            taskmap[tasks - 1 - k] = taskmap[tasks - mtasks];
                            mtasks--;
                            cpuload[i] += yield * taskcpu[t];
                            memload[i] += taskmem[t];
                            taskhost[t] = i;
        ...
        // if there are tasks left we failed and need to
        // lower the upper bound.  Otherwise we succeeded and
        // need to raise the lower bound and note our successful
        // configuration in case future attempts do not return success
        if (ctasks || mtasks) {
            yieldub = yield;
        } else {
            yieldlb = yield;
...
// want to do something like balanced best fit, maximize maxnorm while
// minimizing distance from min norm.  
void MCR_scheduler(
    int hosts, int tasks, float *taskcpu, float *taskmem,               //Input
    int *taskhost                                                       //Output
) {
    ...
    float cpuload[hosts], memload[hosts];
    int minIdx;
    float cpuSum, memSum, minVal;
    int staskhost[tasks];

    while(yieldub - yieldlb > 0.001 && yieldub > bestyield) {
        ...
        for (i = 0; i < tasks; i++) {
            ...
            minVal = 2.0;
            for (j = 0; j < hosts; j++) {
                cpuSum = cpuload[j] + yield * taskcpu[t];
                memSum = memload[j] + taskmem[t];
                if (cpuSum <= 1.0 && memSum <= 1.0) {
                    if (cpuSum > memSum) {
                        if (cpuSum < minVal) {
                            minIdx = j;
                            minVal = cpuSum;
            ...
            if (-1 == minIdx) { // couldn't schedule task
                break;
            }
            cpuload[minIdx] += yield * taskcpu[t];
            memload[minIdx] += taskmem[t];
            taskhost[t] = minIdx;
        }
...
// makes placement decisions based on minimizing max norm
void MCS_scheduler(
    int hosts, int tasks, float *taskcpu, float *taskmem,               //Input
    int *taskhost                                                       //Output
) {
...
// assignes values to tasks based on number of hosts upon which they will fit
// and hosts based on how many tasks will fit on them and tries to put least
// valuable task on least valuable host upon which it will fit.
void MCT_scheduler(
    int hosts, int tasks, float *taskcpu, float *taskmem,               //Input
    int *taskhost                                                       //Output
) {
    ...
    float bestyield = 0.0;
    int utasks;
    int hostmap[hosts], taskmap[tasks];
    float hostkey[hosts], taskkey[tasks];
    float cpuload[hosts], memload[hosts];
    int staskhost[tasks];
    ...
    while(yieldub - yieldlb > 0.001 && yieldub > bestyield) {
        ...
        while (utasks) {
            for (i = 0; i < utasks; i++) {
                t = taskmap[i];
                if ((yield * taskcpu[t]) > taskmem[t]) {
                    taskkey[t] = 1.0 - (yield * taskcpu[t]);
                } else {
                    taskkey[t] = 1.0 - taskmem[t];
                }
            }
            for (i = 0; i < hosts; i++) {
                if (cpuload[i] > memload[i]) {
                    hostkey[i] = 1.0 - cpuload[i];
                } else {
                    hostkey[i] = 1.0 - memload[i];
                }
            }
            for (i = 0; i < utasks; i++) {
                t = taskmap[i];
                for (j = 0; j < hosts; j++) {
                    if (cpuload[j] + yield * taskcpu[t] <= 1.0 &&
                        memload[j] + taskmem[t] <= 1.0
                    ) {
                        taskkey[t] = taskkey[t] + 1.0;
                        hostkey[j] = hostkey[j] + 1.0;
                    }
                }
            }
            sortmap(taskmap, taskkey, utasks, SORT_ASC);
            sortmap(hostmap, hostkey, hosts, SORT_ASC);
            t = taskmap[0];

            if (taskkey[t] < 1.0) { // value < 1.0 implies cannot be scheduled
                break;
            }
            utasks--;
            taskmap[0] = taskmap[utasks];
            for (i = 0; i < hosts; i++) {
                h = hostmap[i];
                if (cpuload[h] + yield * taskcpu[t] <= 1.0 &&
                    memload[h] + taskmem[t] <= 1.0
                ) {
                    cpuload[h] += yield * taskcpu[t];
                    memload[h] += taskmem[t];
                    taskhost[t] = h;
...
// makes placement decisions based on minimizing L2 norm
void MCU_scheduler(
    int hosts, int tasks, float *taskcpu, float *taskmem,               //Input
    int *taskhost                                                       //Output
) {
    ...
    while(yieldub - yieldlb > 0.001 && yieldub > bestyield) {
        ...
        for (i = 0; i < tasks; i++) {
            taskmap[i] = i;
            taskkey[i] = (yield * taskcpu[i]) * (yield * taskcpu[i]) +
                taskmem[i] * taskmem[i];
        }
        sortmap(taskmap, taskkey, tasks, SORT_DESC);
        ...
        for (i = 0; i < tasks; i++) {
            t = taskmap[i];
            for (j = 0; j < hosts; j++) {
                hostkey[j] = (cpuload[j] + yield * taskcpu[t]) *
                    (cpuload[j] + yield * taskcpu[t]) +
                    (memload[j] + taskmem[t]) * (memload[j] + taskmem[t]);
            }
            sortmap(hostmap, hostkey, hosts, SORT_ASC);
            for (j = 0; j < hosts; j++) {
                h = hostmap[j];
...
// maximizes L2 norm, shouldn't be good, but actually does okay...
void MCV_scheduler(
    int hosts, int tasks, float *taskcpu, float *taskmem,               //Input
    int *taskhost                                                       //Output
) {
    ...
    while(yieldub - yieldlb > 0.001 && yieldub > bestyield) {
        ...
        for (i = 0; i < tasks; i++) {
            ...
            sortmap(hostmap, hostkey, hosts, SORT_DESC);
            for (j = 0; j < hosts; j++) {
                h = hostmap[j];
                if (cpuload[h] + yield * taskcpu[t] <= 1.0 &&
                    memload[h] + taskmem[t] <= 1.0
...
// tasks in desc order by largest, keeps 2 logical lists of all hosts ordered 
// by computation and memory respectively, puts M tasks on host with smallest
// M, C tasks on host with smallest C
void MCW_scheduler(
    int hosts, int tasks, float *taskcpu, float *taskmem,               //Input
    int *taskhost                                                       //Output
) {
    ...
    float yield;
    float bestyield = 0.0;
    int chostmap[hosts], mhostmap[hosts], taskmap[tasks];
    float taskkey[tasks];
    float cpuload[hosts], memload[hosts];
    int staskhost[tasks];
        ...
        for (i = 0; i < tasks; i++) {
            ...
            for (j = 0; j < hosts; j++) {
                ...
                if (cpuload[h] + yield * taskcpu[t] <= 1.0 &&
                    ...
                    memload[h] += taskmem[t];
                    taskhost[t] = h;
                    sortmap(chostmap, cpuload, hosts, SORT_ASC);
                    sortmap(mhostmap, memload, hosts, SORT_ASC);
                    break;
                }
            }
            if (j == hosts) {
...
// tasks by desc order largest attr. Keeps 2 lists of hosts sorted in desc order
// by comp and mem resp. Puts C tasks on host with largest M, M tasks on host
// with largest C.
void MCX_scheduler(
    int hosts, int tasks, float *taskcpu, float *taskmem,               //Input
    int *taskhost                                                       //Output
        ...
        for (i = 0; i < tasks; i++) {
            ...
            for (j = 0; j < hosts; j++) {
                ...
                if (cpuload[h] + yield * taskcpu[t] <= 1.0 &&
                    ...
                    memload[h] += taskmem[t];
                    taskhost[t] = h;
                    sortmap(chostmap, cpuload, hosts, SORT_DESC);
                    sortmap(mhostmap, memload, hosts, SORT_DESC);
                    break;
                }
            }
            if (j == hosts) {
...
// tasks in desc order, hosts in asc by sum of cpu and memory load
void MCY_scheduler(
    int hosts, int tasks, float *taskcpu, float *taskmem,               //Input
    int *taskhost                                                       //Output
) {
...
// tasks in desc order by sum, hosts in asc order by largest
void MCZ_scheduler(
    int hosts, int tasks, float *taskcpu, float *taskmem,               //Input
    int *taskhost                                                       //Output
) {
    ...
    while(yieldub - yieldlb > 0.001 && yieldub > bestyield) {
        ...
        for (i = 0; i < tasks; i++) {
            taskmap[i] = i;
            taskkey[i] = (yield * taskcpu[i]) + taskmem[i];
        }
        sortmap(taskmap, taskkey, tasks, SORT_DESC);
...
{"VP_CHERUKI",   VP_scheduler,         "CHERUKI", NULL, NULL},
